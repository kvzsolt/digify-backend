package hu.progmasters.blog.controller.chatgpt;
...
public class ChatGptTest {
    ...
    private static final String CHATGPT_CHAT_ENDPOINT = GPT_MAPPING + "/chat";
    private static final String CHATGPT_PICTURE_GENERATING_ENDPOINT = GPT_MAPPING + "/generate";
    private static final String CHATGPT_PICTURE_GENERATING_BY_TITLE = GPT_MAPPING + "/generate/1";
    private static final String CHATGPT_PICTURE_GENERATING_BY_TITLE_NOT_EXIST = GPT_MAPPING + "/generate/2";
    @Test
    void test_chatGptChatSuccessful() throws Exception {
        ...
        mockMvc.perform(post(CHATGPT_CHAT_ENDPOINT)
    ...
    void test_chatGptGeneratePictureSuccessful() throws Exception {
        ...
        String inputCommand = "{\n" +
                "    \"prompt\": \"Big city\" \n" +
                "}";
        mockMvc.perform(post(CHATGPT_PICTURE_GENERATING_ENDPOINT)
                        .with(user("tesztElek").password("$2a$10$RVP8Q2ybES8dJVGbJP54xehMmrzetBxHJZzNNhvBvRmv2gl7bkNt2").roles("USER"))
                        .contentType(MediaType.APPLICATION_JSON_VALUE)
                        .content(inputCommand))
                .andExpect(status().isOk());
    ...
    void test_chatGptGeneratePictureByTitleSuccessful() throws Exception {
        addUser();
        TestPost();
        mockMvc.perform(post(CHATGPT_PICTURE_GENERATING_BY_TITLE)
                        .with(user("tesztElek").password("$2a$10$RVP8Q2ybES8dJVGbJP54xehMmrzetBxHJZzNNhvBvRmv2gl7bkNt2").roles("USER"))
                        .contentType(MediaType.APPLICATION_JSON_VALUE))
                .andExpect(status().isOk());
    }
    ...
    void test_chatGptGeneratePictureByTitlePostNotExist() throws Exception {
        ...
        String inputCommand = "{\n" +
                "    \"message\": \"Hello gpt\" \n" +
                "}";
        mockMvc.perform(post(CHATGPT_PICTURE_GENERATING_BY_TITLE_NOT_EXIST)
                        .with(user("tesztElek").password("$2a$10$RVP8Q2ybES8dJVGbJP54xehMmrzetBxHJZzNNhvBvRmv2gl7bkNt2").roles("USER"))
                        .contentType(MediaType.APPLICATION_JSON_VALUE)
                        .content(inputCommand))
