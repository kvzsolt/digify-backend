...
@Transactional
public class UserSubscribeNewsletter {
    ...
    private static final String USER_PROFILE_SUBSCRIBE_NEWSLETTER = ACCOUNT_MAPPING + "/profile/newsletter/1";
    private static final String USER_PROFILE_NOT_EXIST_SUBSCRIBE_NEWSLETTER = ACCOUNT_MAPPING + "/profile/newsletter/2";


    @Test
    void test_newsletterSubscribeSuccessful() throws Exception {
        createDb();
        String inputCommand = "{\n" +
                "    \"newsletter\": true \n" +
                "}";
        mockMvc.perform(put(USER_PROFILE_SUBSCRIBE_NEWSLETTER)
    ...
    void test_profilNotExist() throws Exception {
        ...
        String inputCommand = "{\n" +
                "    \"newsletter\": false \n" +
                "}";
        mockMvc.perform(put(USER_PROFILE_NOT_EXIST_SUBSCRIBE_NEWSLETTER)
                        .with(user("testUser").password("testPassword").roles("USER"))
                        .contentType(MediaType.APPLICATION_JSON_VALUE)
                        .content(inputCommand))
